 ...
 * trakt.tv 剧集更新提醒
 *
 * 环境变量: TRAKT, 值: {"proxy": true, "user": "nat_strosin", "cookie": "..."}
 */
...
!(async () => {
    ...
    await axios.get(url, config).then(async (resp) => {
        ...
        const posts = root.querySelectorAll(".posters");

        if (posts.length == 0) {
            await require("./sendNotify.js").sendNotify(`#${$.name}: 未找到数据`, `请检查是否正确设置Cookie`, {}, "");
        } else {
