/**
 * cron: 0 0 * * *
 * 检测 https://kox.moe/ 是否有推送的漫画
 *
 * 环境变量: KOXMOE_COOKIE, 值: 对应cookie
 */
const $ = new Env('Kindle漫画');
const ENV = 'KOXMOE_COOKIE';
...
!(async () => {
    ...
    await axios.get('https://kox.moe/myrecord.php', config).then(async (resp) => {
        ...
        if (trList.length > 3) {
            const title = trList[3].querySelectorAll('td')[1].rawText.trim();
