...
!(async () => {
    ...
    const data = parseInt($.getdata($.name) || 0)
    ...
    await axios.get("https://api.bilibili.com/x/polymer/web-dynamic/v1/feed/all?type=video&platform=web&page=1", config).then(async (resp) => {
        ...
        for (const item of resp.data.data.items) {
            ...
            const url = item.modules.module_dynamic.major.archive.jump_url;
            if (flag == 0) {
                flag = ts
            }

            if (ts <= data) {
                $.log(`发现已通知的时间戳: ${title}, ts: ${ts}`);
                break;
            }
            $.msg(`发现更新: ${author}, ${title}`);
            content += `\n${author}: ${title} (https:${url})`
        }
        if (content) {
            await require('./sendNotify.js').sendNotify(`${$.name} 有更新`, content, {}, '').then(() => {
                $.setdata(flag, $.name);
            });
        }
    });
